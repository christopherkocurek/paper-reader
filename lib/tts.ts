...
export type ElevenLabsVoice = {
  ...
  preview_url?: string;
  /**
   * True if this voice is owned by the account (cloned or generated by the
   * user). Free-tier API keys can ONLY use owned voices — library voices
   * (premade, professional, shared) return HTTP 402 "paid_plan_required".
   */
  is_owner?: boolean;
};
 ...
 *
 * Note: the response includes BOTH voices the user owns (usable on any plan)
 * AND library voices (premade/professional) which require a paid plan for
 * API usage. Callers should check `is_owner` to filter.
 */
